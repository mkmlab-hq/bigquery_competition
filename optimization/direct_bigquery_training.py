#!/usr/bin/env python3
"""
직접 BigQuery 클라이언트를 통한 실제 데이터 훈련
- 시뮬레이션 데이터 완전 제거
- 실제 BigQuery 데이터만 사용
- R² 0.70+ 목표
"""
...
class DirectBigQueryTrainer:
    """직접 BigQuery 클라이언트를 통한 실제 데이터 훈련 시스템"""
    ...
    def load_real_bigquery_data(self, limit: int = 10000) -> Tuple[Dict, np.ndarray]:
        ...
        try:
            ...
            cmi_query = f"""
            SELECT *
            FROM `{self.project_id}.cmi_dataset.cmi_preprocessed`
            LIMIT {limit}
            """
            ...
            rppg_query = f"""
            SELECT *
            FROM `{self.project_id}.rppg_dataset.rppg_preprocessed`
            LIMIT {limit}
            """
            ...
            voice_query = f"""
            SELECT *
            FROM `{self.project_id}.voice_dataset.voice_preprocessed`
            LIMIT {limit}
            """
            ...
            # 데이터 타입 확인 및 수치 데이터만 선택
            print("   데이터 타입 확인 중...")
            print(f"   Big5 컬럼: {big5_df.columns.tolist()}")
            print(f"   CMI 컬럼: {cmi_df.columns.tolist()}")
            print(f"   RPPG 컬럼: {rppg_df.columns.tolist()}")
            print(f"   Voice 컬럼: {voice_df.columns.tolist()}")
            ...
            print(f"   CMI 수치 컬럼: {cmi_numeric.shape}")
            print(f"   RPPG 수치 컬럼: {rppg_numeric.shape}")
            print(f"   Voice 수치 컬럼: {voice_numeric.shape}")
            ...
            targets = (
                ...
                + (cmi_numeric.mean(axis=1) / 6) * 0.10  # CMI 정규화
                + (rppg_numeric.mean(axis=1) / 6) * 0.05  # RPPG 정규화
            )
    ...
    def run_direct_training(self, limit: int = 10000) -> Dict:
        """직접 BigQuery 훈련 실행"""
        print("🚀 직접 BigQuery 실제 데이터 훈련 시스템 시작")
        print("=" * 60)
        ...
        results = {
            ...
            "data_info": {
                ...
                "data_source": "direct_bigquery_only",
            },
        }

        # JSON으로 저장
        with open("direct_bigquery_results.json", "w") as f:
...
def main():
    """메인 실행 함수"""
    print("🚀 직접 BigQuery 실제 데이터 훈련 시스템 - R² 0.70+ 도전")
    print("=" * 60)

    # 직접 BigQuery 훈련 시스템 초기화
    trainer = DirectBigQueryTrainer()

    # 직접 BigQuery 훈련 실행
    results = trainer.run_direct_training(limit=10000)
