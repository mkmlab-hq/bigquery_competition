...
class RobustEnsembleTrainer:
    ...
    def prepare_robust_data(self, multimodal_data: Dict, targets: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """강건한 데이터 준비"""
        print("🔄 강건한 데이터 준비 중...")
    ...
    def select_stable_models(self, model_scores: Dict, stability_threshold: float = 0.02) -> List[str]:
        """안정적인 모델들 선택"""
        print("🔄 안정적인 모델들 선택 중...")
        ...
        for name, scores in model_scores.items():
            if scores is not None:
                ...
                if scores["cv_std"] < stability_threshold and scores["cv_mean"] > 0.3:
                    ...
                    print(f"   ✅ {name}: R² = {scores['cv_mean']:.4f} (±{scores['cv_std']:.4f})")

        print(f"✅ {len(stable_models)}개 안정적인 모델 선택 완료")
        return stable_models

    def create_robust_ensemble(self, X: np.ndarray, y: np.ndarray, model_scores: Dict, stable_models: List[str]) -> Dict:
        """강건한 앙상블 생성"""
        print("🔄 강건한 앙상블 생성 중...")
    ...
    def run_robust_training(self, limit: int = 10000) -> Dict:
        """강건한 훈련 실행"""
        print("🚀 강건한 앙상블 훈련 시스템 시작")
        print("=" * 60)
        ...
        # 6. 강건한 앙상블 생성
        ensemble_results = self.create_robust_ensemble(X, y, model_scores, stable_models)
        ...
        # JSON으로 저장
        with open("robust_ensemble_results.json", "w") as f:
...
def main():
    """메인 실행 함수"""
    print("🚀 강건한 앙상블 훈련 시스템 - R² 0.70+ 도전")
    print("=" * 60)
    ...
    # 강건한 훈련 실행
    results = trainer.run_robust_training(limit=10000)
