...
def create_final_models():
    ...
    # 최종 모델 생성
    final_model = FinalOptimizedModel(device)
    ...
    final_model.add_model("neural_net_1", model1, "neural_network", weight=1.0)

    # 모델 2: 다른 하이퍼파라미터
    model2 = TransferLearningMultimodalNet(
        hidden_dim=128, dropout_rate=0.4, use_pretrained=False
    )
    final_model.add_model("neural_net_2", model2, "neural_network", weight=1.0)

    # 2. Scikit-learn 모델들 추가 (Lasso 제외)
    ...
    final_model.add_model("ridge", Ridge(alpha=1.0), "sklearn", weight=1.0)
...
def test_final_model():
    ...
    # 최종 모델 생성 및 훈련
    final_model = create_final_models()
    final_model.train_models(X_train, y_train, X_val, y_val)
    ...
    results = {
        "final_model_test": {
            ...
            "test_rmse": test_rmse,
            "test_mae": test_mae,
            "model_count": len(final_model.models),
            "weights": final_model.weights,
        }
    }

    with open("final_model_test_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("✅ 결과 저장 완료!")
    print("📁 final_model_test_results.json")
